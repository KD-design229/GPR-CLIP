...
from clients.clientFedSAM import clientFedSAM

class FedSAM(ServerBase):
    def __init__(self, args, modelObj, run_times, logger):
        super(FedSAM, self).__init__(args, modelObj, run_times, logger)
        
        # 1. 划分数据集
        self.dataset_division()
        
        # 2. 设置客户端（使用 clientFedSAM）
        self.set_clients(args, self.global_model, clientFedSAM)
        
        self.logger.info(f"FedSAM Server Initialized with sam_rho={args.sam_rho}")

    def train(self):
        # 复用与 FedAvg 相同的完整训练流程
